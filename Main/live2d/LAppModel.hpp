...
public:
     ...
     */
    Csm::CubismMotionQueueEntryHandle StartMotion(const Csm::csmChar *group, Csm::csmInt32 no, Csm::csmInt32 priority, OnMotionStartCallback onStartMotionHandler = NULL, OnMotionFinishCallback onFinishedMotionHandler = NULL);
     ...
     * @return                                  開始したモーションの識別番号を返す。個別のモーションが終了したか否かを判定するIsFinished()の引数で使用する。開始できない時は「-1」
     */
    Csm::CubismMotionQueueEntryHandle StartRandomMotion(const Csm::csmChar *group, Csm::csmInt32 priority, OnMotionStartCallback onStartMotionHandler = NULL, OnMotionFinishCallback onFinishedMotionHandler = NULL);
